...
public class UserServiceImpl implements UserService {
    ...
    public SimpleResponse update(Long restaurantId, Long userId, UserRequest request) {
        ...
        return new SimpleResponse("FAIL","Employee with id: "+userId+" failed to update!");
    }
    ...
    public SimpleResponse delete(Long restaurantId, Long userId) {
        User user = repository.findById(userId)
                .orElseThrow(() ->
                        new NoSuchElementException("Restaurant with id:" + userId + " not found!"));
        if (user.getId().equals(userId)){
    ...
    public SimpleResponse assign(AcceptRequest request) {
        Restaurant restaurant = restaurantRepository.findById(request.restaurantId()).orElseThrow(() ->
                new NoSuchElementException("Restaurant with id: " + request.restaurantId() + " not found!"));
    ...
    public AuthenticationResponse register(UserRequest request, Long restaurantId) {
        ...
        if(repository.existsByEmail(request.email())){
            throw new RuntimeException(String.format("User with email: %s already exists!", request.email()));
        }
        List<UserResponse> users = repository.getAllUsers(restaurantId);
        if (users.size() > 15){
            throw new RuntimeException("Restaurant employees are full (15) . Sorry!");
        }
        ...
        if (user.getRole() == Role.CHEF) {
            if (age < 25 || age > 45 || user.getExperience() < 2) {
                throw new RuntimeException("Chef's age should be between 25-45 and experience >= 2");
            }
        } else if (age < 18 || age > 30 || user.getExperience() < 1) {
            throw new RuntimeException("Waiter's age should be between 18-30 and experience >= 1");
        }
